...
public partial class MainWindow : Window
{
    ...
    {
        var pattern = @"^\d{2} \d{2} \d{6}$";
        var validationResult = !Regex.IsMatch(DataFromApi, pattern);
        ValidationResultTBlock.Text = validationResult ? "Номер карты содержит запрещенные символы" : "Номер карты не содержит запрещенные символы";
